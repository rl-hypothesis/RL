...
def test_groups(group, grp_type, names, hierarchy, dimension, top_n, num_hyps, approach,\
agg_type, test_arg, users_1, support, alpha, verbose=False):
    ...
    theName = [name_groups(group[i]) for i in range(len(group))]

    groups = []
    nameGrp_2_index = []
    group_2_users = []

    for j,grp in enumerate(theName):
        liste = []
        dicte = dict()
        dicte_2 = dict()

        for i in hierarchy[grp]:
            df = names[j][i]
    ...
    #groups =  [ [names[j][i] for i in hierarchy[grp]  ] for j,grp in enumerate(theName) ]

    #groups = [ group_handler.groups(d, grp_type[i]) for i,d in enumerate(df)]
    #groups = [ group_handler.groups(d, grp_type[i]) for i,d in enumerate(group)]
    ...
    groups_ = []
    for grp in groups:
        supp_idx = list(range(len(grp)))
        dicte = dict()
        dicte_2 = dict()

        for idx,df in enumerate(grp):
            name = name_groups(df)
            if name in dicte:
                supp_idx.remove(idx)
            else:
                dicte[name] = df
                dicte_2[name] = set(df.cust_id.unique())

        nameGrp_2_index.append( dicte )
        group_2_users.append( dicte_2 )

        #if len(grp) != 1:
        grp = [df for i,df in enumerate(grp) if i in supp_idx]
        #grp = list( np.array(grp)[supp_idx] )
